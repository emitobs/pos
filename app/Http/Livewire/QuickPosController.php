...
class QuickPosController extends Component
{
    ...
    public function render()
    {
        ...
        $this->refreshTotal();
        $this->updateRemainingAmountToPay();

        return view(
            'livewire.QuickPos.component',
            [
                'beepers' => $beepers,
    ...
    private function addToCart(Product $product, int $quantity, ?string $detail)
    {
        ...
        $this->cart_local = $this->cartService->addProduct($this->cart_local, $product, $quantity, $detail, $product->image);
    }
    ...
    public function addPay()
    {
        ...
        if ($totalPayments + $this->remainingAmount > $this->cart_total) {
            $this->emit('error-payment-exceeds-total', 'Solo se permite efectivo para pagos que exceden el total');
            return;
        }

        if (empty($this->payments) || $this->payment_method_selected == 2) { //Methos 2 es Handy
            $this->payments[] = ["amount" => $this->remainingAmount, "method_id" => $this->payment_method_selected];
        } else {
            $found = false;

            // Buscar si ya existe el method_id en el array
            foreach ($this->payments as &$payment) {
                if ($payment['method_id'] == $this->payment_method_selected) {
                    // Sumar al amount existente
            ...
            // Si no se encontró, añadir un nuevo elemento al array
            if (!$found) {
                $this->payments[] = ["amount" => $this->remainingAmount, "method_id" => $this->payment_method_selected];
            }
        ...
        $totalPayments += $this->remainingAmount;
    ...
    public function updateRemainingAmountToPay()
    {
        $totalPayments = array_reduce($this->payments, function ($carry, $payment) {
            return $carry + $payment['amount'];
        }, 0);

        $this->remainingAmount = $this->cart_total - $totalPayments;
    }

    public function updateCart($key, $field, $value)
    {
        // Asegúrate de validar los datos aquí según tus necesidades.

        // Actualiza el carrito en la sesión o donde lo tengas almacenado.
        $this->cart[$key][$field] = $value;
        session()->put('cart_local', array_reverse($this->cart, true));

        // Para efectos de visualización inmediata
        $this->cart = array_reverse(session()->get('cart_local'), true);
    }
}
